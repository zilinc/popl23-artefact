...
thm Random_seed_AllRefine.Random_seed_cogent_shallow.corres_shallow_C_sum[no_vars]
thm random_seed.rand_with_seed'_def

thm Random_seed_cogent_shallow_def

lemma corres_shallow_C_sum_no_assms:
...
(\<And>i \<gamma> v' \<Gamma>' \<sigma> st.
    ...
    update_sem_init.corres upd_abs_typing abs_repr Random_seed.state_rel 
                    ...
                    (do x <- random_seed.rand_with_seed' v';
                              gets (\<lambda>s. x)
                     od)
     \<xi>_0 \<gamma> (assoc_lookup [(''rand_with_seed'', Random_seed_TypeProof.rand_with_seed_type), (''sum'', Random_seed_TypeProof.sum_type)] (0, [], {}, TUnit, TUnit)) \<Gamma>' \<sigma>
     st) \<Longrightarrow>
...
value_sem.matches val_abs_typing \<Xi> [vv\<^sub>m] [Some (fst (snd (snd (snd Random_seed_TypeProof.sum_type))))] \<Longrightarrow>
correspondence_init.corres_shallow_C abs_repr upd_abs_typing abs_upd_val rename Random_seed.state_rel (Random_seed_Shallow_Desugar.sum vv\<^sub>s) Random_seed_TypeProof.sum
 (random_seed.sum' uv\<^sub>C) \<xi>_0 \<xi>\<^sub>m \<xi>\<^sub>p [uv\<^sub>m] [vv\<^sub>m] \<Xi> [Some (fst (snd (snd (snd Random_seed_TypeProof.sum_type))))] \<sigma> s
  [cogent_C_heap]"
