...
record ('s, 'time, 'timeout, 'timer_mode) driver =
   get_time :: "'s \<Rightarrow> 'time" 
   initialize :: "'s \<Rightarrow> 's"
   ...
   iniDeviceInv :: "'s \<Rightarrow> bool"

locale dataInvariant =
  fixes dr ::  "('s1, 't1, 'to1, 'tm1) driver"  
  assumes initDI : "iniDeviceInv dr s \<Longrightarrow> stateInv dr (initialize dr s)"
...
locale driver_morphism_laws =  
  ...
  fixes dr1 ::  "('s1, 't1, 'to1, 'tm1) driver"
  fixes dr2 :: "('s2, 't2, 'to2, 'tm2) driver" 
  fixes mor :: "('s1, 't1, 'to1, 'tm1, 
                 's2, 't2, 'to2, 'tm2) driver_morphism_rec"   (structure) 
  assumes \<alpha>get_time : "\<And> s. stateInv dr1 s \<Longrightarrow> get_time dr2 (\<alpha> s) = \<alpha>time (get_time dr1 s)"
  assumes \<alpha>initialize : "initialize dr2 (\<alpha> s) = \<alpha> (initialize dr1 s)"
  ...
  assumes \<alpha>iniDeviceInv : "iniDeviceInv dr1 s \<Longrightarrow> iniDeviceInv dr2 (\<alpha> s)"
...
definition abs_driver :: "(abstr_state, nat, nat, timer_mode) driver"
  where
  "abs_driver = 
\<lparr> get_time = (\<lambda> s. (timer_e_low_hi (deviceState s))  * (ns_in_us :: nat)),

 initialize = (\<lambda> s. s \<lparr> deviceState := (deviceState s) \<lparr>     
        timer_e_low_hi := 0
...
set_timeout = (\<lambda> s timeout periodic. 
  let deviceState' = (deviceState s) \<lparr>
   ...
   , iniDeviceInv = (\<lambda>s. disable (driverState s) \<and> Not (timer_a_en (deviceState s)))
...
type_synonym ('s, 'time, 'timeout, 'timer_mode) driver_abstr = 
   "('s, 'time, 'timeout, 'timer_mode, 
     abstr_state, nat, nat, timer_mode) driver_morphism_rec"

locale is_refinement = 
  driver_morphism_laws dr abs_driver mor
+ dataInvariant dr
  for dr :: "('s, 'time, 'timeout, 'timer_mode) driver"
 and mor :: "('s, 'time, 'timeout, 'timer_mode) driver_abstr"
