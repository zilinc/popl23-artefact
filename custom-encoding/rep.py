...
corressetupfile = 'Generated_CorresSetup.thy'
...
def replaceLine(filename, lineIn, lineOut):
...
# Remove the (redundant) definition of CEntry
replaceLine(cfilename, "typedef struct { int id; Stuff stuff; int value; } CEntry;", 
   "typedef Entry CEntry;" )


replaceLine(corressetupfile, 'local_setup \<open> local_setup_getter_correctness "generated.c" \<close>', '')
replaceLine(corressetupfile, '(* Proving correctness of getters *)', 
    '(* Getter specification is not supported when abstract types are involved *)')
